...
function initWeb3Modal() {
    try {
        ...
        // Only add WalletConnect if available
        if (typeof window.WalletConnectProvider !== 'undefined') {
            providerOptions.walletconnect = {
                package: window.WalletConnectProvider.default,
                options: {
                    projectId: "a7a4da3c4d7e3b5f9c8e6a2b1d0f3e5c",
                    rpc: {
            ...
            console.warn('⚠️ WalletConnect not loaded, using injected provider only');
        }

        web3Modal = new window.Web3Modal.default({
            cacheProvider: false, // Changed to false to fix reconnection issues
            providerOptions,
...
async function connectWallet() {
    try {
        ...
        // First try: Check if MetaMask is installed and use it directly
        if (typeof window.ethereum !== 'undefined') {
            console.log('✅ MetaMask detected, connecting directly...');
            ...
            try {
                ...
                console.log('⚠️ MetaMask connection failed, trying Web3Modal...', metaMaskError);
            }
        }

        // Second try: Use Web3Modal for other wallets
        if (!web3Modal) {
            if (!initWeb3Modal()) {
        ...
        console.log('✅ Connected via Web3Modal:', address);
    } catch (error) {
        ...
        if (error.message && error.message.includes('User closed modal')) {
            ...
            showNotification('Request already pending. Check your wallet.', 'error');
        } else if (error.code === 4001) {
            showNotification('Connection rejected by user', 'error');
        } else {
            console.error('Full error:', error);
            showNotification('Connection failed. Try refreshing the page.', 'error');
        }
    }
}
...
// Note: Auto-reconnect disabled to prevent connection issues
// Users need to click "Connect Wallet" each time
