    ...
    try {
        ...
        if (typeof window.ethereum !== 'undefined') {
            try {
                ...
                console.log('⚠️ MetaMask failed, trying Web3Modal...', metaMaskError);
            }
        }
        ...
        if (error.code === -32002) {
            showNotification('Request pending. Check your wallet.', 'error');
        } else if (error.code === 4001) {
            showNotification('Connection rejected', 'error');
        } else if (!error.message.includes('User closed modal')) {
            showNotification('Connection failed. Try refreshing.', 'error');
        }
    }
}
